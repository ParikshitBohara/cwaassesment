...
export default function EscapeRoomPage() {
  ...
  const runUserCode = () => {
    try {
      ...
      setStage3(ok);
      if (!ok) alert("Keep trying! Make sure your array matches the selected challenge.");
    } catch (e: any) {
  ...
  const saveProgress = async () => {
    if (!studentNumber || studentNumber === "sXXXXXXX") {
  ...
  const IconReset = () => (
    <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden className="inline-block mr-1">
      <path fill="currentColor" d="M13 3a9 9 0 1 0 8.49 6.26l-1.9.62A7 7 0 1 1 13 5v3l5-4-5-4v4z" />
    </svg>
  );
      ...
      <div className="max-w-6xl mx-auto bg-black/50 rounded-2xl p-6 shadow-xl text-white">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <label className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-24 px-3 py-2 rounded bg-white/10 outline-none"
              />
            </label>

            <div className="flex items-center gap-2">
              <button onClick={startTimer} className="px-3 py-2 rounded bg-emerald-600 hover:bg-emerald-500">
                <IconPlay /> Start
              </button>
              <button onClick={stopTimer} className="px-3 py-2 rounded bg-yellow-600 hover:bg-yellow-500">
                <IconPause /> Pause
              </button>
              <button onClick={reset} className="px-3 py-2 rounded bg-rose-600 hover:bg-rose-500">
                <IconReset /> Reset
              </button>
            </div>
        ...
        <section className="mt-6 grid lg:grid-cols-2 xl:grid-cols-4 gap-6">
          ...
          <article className="bg-white/5 rounded-xl p-4">
            ...
            <div className="mt-3 flex items-center gap-2">
              <button onClick={runUserCode} className="px-3 py-2 rounded bg-purple-600 hover:bg-purple-500">
                Run & Validate
              </button>
        ...
        <footer className="mt-6 flex flex-wrap items-center gap-3">
          <button
            ...
            aria-label="Save progress to database"
          >
            Save Progress
          </button>
          <button
            ...
            aria-label="Load last saved progress"
          >
            Load Last Save
          </button>
...
function StatusBadge({ ok }: { ok: boolean }) {
  return (
    <span
      className={`inline-block text-sm px-2 py-1 rounded ${ok ? "bg-emerald-600" : "bg-gray-600"}`}
      aria-live="polite"
    >
